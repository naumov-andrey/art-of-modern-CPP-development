...
public:
  AirportCounter() {
    for (size_t i = 0; i < data_.size(); ++i) {
      data_[i].first = static_cast<TAirport>(i);
      data_[i].second = 0;
    }
  }
    ...
    AirportCounter()
  {
    for (auto it = begin; it != end; ++it) {
      ++data_[static_cast<size_t>(*it)].second;
    }
  }

  size_t Get(TAirport airport) const { return data_[static_cast<size_t>(airport)].second;}

  void Insert(TAirport airport) { ++data_[static_cast<size_t>(airport)].second;}

  void EraseOne(TAirport airport) {
    if (data_[static_cast<size_t>(airport)].second != 0) {
      --data_[static_cast<size_t>(airport)].second;
    }
  }

  void EraseAll(TAirport airport) { data_[static_cast<size_t>(airport)].second = 0;}

  using Item = pair<TAirport, size_t>;
  using Items = array<Item, static_cast<size_t>(TAirport::Last_)>;

  const Items& GetItems() const { return data_;}
